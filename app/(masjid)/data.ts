...
export const getMosqueOnboardingData = cache(async (orgId: string) => {
  ...
  const { data: mosque } = await supabase
    .from("mosques")
    .select("id, name, address, city, state, phone, email, timezone, app_name, logo_url, brand_color, accent_color, secondary_color, font_theme, calculation_method, school, midnight_mode, latitude_adjustment_method, prayer_tune, shafaq, reels_scope, onboarding_progress, onboarding_status, stripe_account_id, subscription_status")

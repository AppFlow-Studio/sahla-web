...
async function handleSaasCheckoutCompleted(session: Stripe.Checkout.Session) {
  ...
  // Mosque is now READY: payment succeeded, subscription active. The Sahla
  // team still needs to build the binary and flip it to "live" via the
  // admin "Mark as Live" action — at which point launched_at is set.
  await supabase
    .from("mosques")
    .update({
      subscription_status: "active",
  ...
  // Notify dev team — this masjid needs an app build before going live.
  ...
  await notifySlack(
    `📦 *${mosqueName}* is READY for launch — paid and waiting on build.\n• Tier: ${tier || "unknown"} (${amount})\n• Mosque ID: ${mosqueId}\n• Slug: ${liveData?.slug || "—"}`
  );
